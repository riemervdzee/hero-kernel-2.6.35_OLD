...
#ifndef CONFIG_OYAFFS_USE_OWN_SORT
...
#ifdef CONFIG_OYAFFS_ALWAYS_CHECK_CHUNK_ERASED
...
#ifdef CONFIG_OYAFFS_CASE_INSENSITIVE
...
{
#ifdef CONFIG_OYAFFS_SHORT_NAMES_IN_RAM
...
#ifdef CONFIG_OYAFFS_TNODE_LIST_DEBUG
	...
	for (blockIterator = endIterator; !alloc_failed && blockIterator >= startIterator;
		...
		for (c = dev->nChunksPerBlock - 1;
			...
			if (!tags.chunkUsed) {
				...
				if (!in ||
#ifdef CONFIG_OYAFFS_DISABLE_LAZY_LOAD
				    !in->valid ||
#endif
				    tags.extraShadows ||
				    (!in->valid &&
...
#ifndef CONFIG_OYAFFS_TNODE_LIST_DEBUG
