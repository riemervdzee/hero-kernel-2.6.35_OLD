...
static ssize_t hero_h2w_print_name(struct switch_dev *sdev, char *buf)
{
...
static void insert_headset(void)
{
	 ...
	 */
	hi->ignore_btn = !gpio_get_value(HERO_GPIO_CABLE_IN2);
...
static void detection_work(struct work_struct *work)
{
	...
	if (gpio_get_value(HERO_GPIO_CABLE_IN1) != 0) {
	...
	gpio_direction_output(HERO_GPIO_CABLE_IN1, 1);
	/* Delay 10ms for pin stable. */
	msleep(10);
	/* Save H2W_CLK */
	clk = gpio_get_value(HERO_GPIO_H2W_CLK_GPI);
	/* Set GPIO_CABLE_IN1 as input */
	gpio_direction_input(HERO_GPIO_CABLE_IN1);
	...
	cable_in1 = gpio_get_value(HERO_GPIO_CABLE_IN1);
...
static enum hrtimer_restart button_event_timer_func(struct hrtimer *data)
{
	...
	if (switch_get_state(&hi->sdev) == HTC_HEADSET) {
		if (gpio_get_value(HERO_GPIO_CABLE_IN2)) {
			if (hi->ignore_btn)
				hi->ignore_btn = 0;
...
static irqreturn_t detect_irq_handler(int irq, void *dev_id)
{
	...
	H2W_DBG("");
	do {
		value1 = gpio_get_value(HERO_GPIO_CABLE_IN1);
		set_irq_type(hi->irq, value1 ?
				IRQF_TRIGGER_LOW : IRQF_TRIGGER_HIGH);
		value2 = gpio_get_value(HERO_GPIO_CABLE_IN1);
...
static irqreturn_t button_irq_handler(int irq, void *dev_id)
{
	...
	H2W_DBG("");
	do {
		value1 = gpio_get_value(HERO_GPIO_CABLE_IN2);
		set_irq_type(hi->irq_btn, value1 ?
				IRQF_TRIGGER_LOW : IRQF_TRIGGER_HIGH);
		value2 = gpio_get_value(HERO_GPIO_CABLE_IN2);
	} while (value1 != value2 && retry_limit-- > 0);
...
static int hero_h2w_probe(struct platform_device *pdev)
{
	int ret;
	unsigned long irq_flags;
	...
	hi->sdev.name = "h2w";
	hi->sdev.print_name = hero_h2w_print_name;
	...
	ret = gpio_request(HERO_GPIO_CABLE_IN1, "h2w_detect");
	if (ret < 0)
		goto err_request_detect_gpio;

	ret = gpio_request(HERO_GPIO_CABLE_IN2, "h2w_button");
	if (ret < 0)
		goto err_request_button_gpio;

	ret = gpio_direction_input(HERO_GPIO_CABLE_IN1);
	if (ret < 0)
		goto err_set_detect_gpio;

	ret = gpio_direction_input(HERO_GPIO_CABLE_IN2);
	if (ret < 0)
		goto err_set_button_gpio;

	hi->irq = gpio_to_irq(HERO_GPIO_CABLE_IN1);
	if (hi->irq < 0) {
		ret = hi->irq;
		goto err_get_h2w_detect_irq_num_failed;
	}

	hi->irq_btn = gpio_to_irq(HERO_GPIO_CABLE_IN2);
	if (hi->irq_btn < 0) {
		ret = hi->irq_btn;
...
err_set_button_gpio:
err_set_detect_gpio:
	gpio_free(HERO_GPIO_CABLE_IN2);
err_request_button_gpio:
	gpio_free(HERO_GPIO_CABLE_IN1);
...
static int hero_h2w_remove(struct platform_device *pdev)
{
	H2W_DBG("");
...
static struct platform_device hero_h2w_device = {
	.name		= "hero-h2w",
};

static struct platform_driver hero_h2w_driver = {
	.probe		= hero_h2w_probe,
	.remove		= hero_h2w_remove,
	.driver		= {
		.name		= "hero-h2w",
		.owner		= THIS_MODULE,
	},
...
static int __init hero_h2w_init(void)
{
	if (!machine_is_hero())
		return 0;
	int ret;
	H2W_DBG("");
	ret = platform_driver_register(&hero_h2w_driver);
	if (ret)
		return ret;
	return platform_device_register(&hero_h2w_device);
}

static void __exit hero_h2w_exit(void)
{
	platform_device_unregister(&hero_h2w_device);
	platform_driver_unregister(&hero_h2w_driver);
}

module_init(hero_h2w_init);
module_exit(hero_h2w_exit);

MODULE_AUTHOR("Laurence Chen <Laurence_Chen@htc.com>");
MODULE_DESCRIPTION("HTC 2 Wire detection driver for hero");
MODULE_LICENSE("GPL");
