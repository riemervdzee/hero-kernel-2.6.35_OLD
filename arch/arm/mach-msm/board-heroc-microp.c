/* arch/arm/mach-msm/board-heroc-microp.c
...
static int heroc_microp_function_init(struct i2c_client *client)
{
...
static struct microp_ops ops = {
	.init_microp_func = heroc_microp_function_init,
};
...
static struct platform_driver capella_cm3602_driver = {
	...
	.driver = {
		.name = "heroc_proximity",
		.owner = THIS_MODULE
	},
};

static int __init heroc_capella_cm3602_init(void)
{
	if (!machine_is_heroc())
		return 0;
...
device_initcall(heroc_capella_cm3602_init);
